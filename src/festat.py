...
class PTAInnerProduct:
    ...
    def inner_product_aNb(self, a, b):
        return sum(
            [
    ...
    def inner_product_TNa(self, a):
        return self.inner_product_aNb(self.Ts, a)

    def inner_product_aKb(self, a, b):
        aNb = self.inner_product_aNb(a, b)
        aKb = aNb

        TNas = self.inner_product_TNa(a)
        TNbs = self.inner_product_TNa(b)
        ...
        if self.pta._commonsignals:
            ...
            for TNa, TNb, TNT, phiinv in zip(
                self.TNas, self.TNbs, self.TNTs, self.phiinvs
            ):
                Sigma = TNT + (np.diag(phiinv) if phiinv.ndim == 1 else phiinv)
