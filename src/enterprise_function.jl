export eccentric_pta_signal,
    ...
    eccentric_pta_signal_components
...
function eccentric_pta_signal_components(
    toas,
    theta::Float64,
    phi::Float64,
    pdist::Float64,
    ...
    term = psrTerm ? PULSAR : EARTH
    tref = Time(tref)
    tEs = Time.(toas)

    return residuals_components_𝒜(mass, n_init, e_init, l0p, dp, psrpos, gwpos, term, tref, tEs)
end